...
export default {
  ...
  computed: {
    showCustomersCount() {
      return this.displayText.toLowerCase() === "business" ? this.businessCustomersCount : this.displayText.toLowerCase() === "individual" ? this.individualCustomersCount : this.customersCount
    }
  },
  ...
  methods: {
    ...
    fetchCustomersCount() {
      this.$devInstance.get("/customers/total-customer-count").then((res) => {
    ...
    fetchProductsCount() {
      this.$devInstance.get("/products/total-product-count").then((res) => this.productsCount = res?.data?.data?.productsCount).catch((err) => console.log(err))
    },
  },
};
</script>
